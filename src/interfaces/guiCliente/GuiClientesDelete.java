	...
	btnDeletarCliente.addActionListener(new ActionListener() {
		...
		public void actionPerformed(ActionEvent e) {
	        ...
	        try {
	            ...
	            }else if (boxDeleteCadastroCliente.isSelected()) {
	                delete.deleteClienteCadastro(cadastroClienteDel);
	            }

	            else if (boxDeleteUnicoPedido.isSelected()) {
	     ...
	     //cpf ou id não existe
			try {
				if(!delete.verificarCpf(allPedidoDel) && sucesso && boxDeleteAllPedido.isSelected()) {
						JOptionPane.showMessageDialog(null, "O cpf não existe", "Erro", JOptionPane.ERROR_MESSAGE);

				}


				if(!delete.verificarCpf(unicoPedidoDel) && sucesso && boxDeleteUnicoPedido.isSelected()) {

					JOptionPane.showMessageDialog(null, "O cpf não existe", "Erro", JOptionPane.ERROR_MESSAGE);

				}else if(!delete.verificarPedido(idPedidoDel) && sucesso && boxDeleteUnicoPedido.isSelected()) {

					JOptionPane.showMessageDialog(null, "O id do pedido não existe", "Erro", JOptionPane.ERROR_MESSAGE);

				}

				if(!delete.verificarCpf(cadastroClienteDel)  && sucesso && boxDeleteCadastroCliente.isSelected()) {
